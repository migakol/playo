    ...
    def create_2d_visualization(self, nodes, title="Game Level Visualization", save_path=None, figsize=(14, 8)):
        ...
        for node in nodes:
            ...
            if node_type == 'center':
                ...
                circle = patches.Circle((x, z), radius=0.3, facecolor='blue', edgecolor='black', linewidth=1.5,
                                        zorder=3)
                ax.add_patch(circle)
            elif node_type == 'entrance':
                # Green rectangle with black outline
                rect = patches.Rectangle((x - 0.1, z - 0.1), 0.4, 0.4, facecolor='green', edgecolor='black',
                                         linewidth=1.5, zorder=3)
                ax.add_patch(rect)
            elif node_type == 'corridor':
                # Orange triangle with black outline
                triangle = patches.Polygon([[x, z + 0.3], [x - 0.3, z - 0.3], [x + 0.3, z - 0.3]],
                                           facecolor='orange', edgecolor='black', linewidth=1.5, zorder=3)
                ax.add_patch(triangle)

            # Add ID in black text inside a small black-outlined box
            ax.text(x + 0.4, z + 0.4, str(node['id']), fontsize=9, fontweight='bold',
